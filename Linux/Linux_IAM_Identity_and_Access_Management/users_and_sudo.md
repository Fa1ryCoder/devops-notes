...
### Real, Effective, Saved, Filesystem UID — четыре вида UID процесса

У каждого процесса не один UID, а четыре. Это фундаментальный механизм ядра который лежит в основе sudo, SUID и любого повышения привилегий:

| | Название | Смысл |
|-|---------|-------|
| **RUID** | Real UID | Кто реально запустил процесс. Не меняется на протяжении жизни процесса (кроме явного вызова `setuid()` от root). |
| **EUID** | Effective UID | От чьего имени ядро проверяет права прямо сейчас при большинстве syscall'ов: `open()`, `read()`, `kill()`, `bind()`. Обычно равен RUID, но меняется при SUID и sudo. |
| **SUID** | Saved UID | Сохранённая копия EUID до его изменения. Позволяет процессу временно сбросить привилегии и вернуть их обратно без участия ядра. |
| **fsUID** | Filesystem UID | Отвечает только за проверку прав при операциях с файлами: `open()`, `stat()`, `chmod()`. Нужен чтобы разделить «кто ты для файловой системы» и «кто ты для всего остального». Пример: NFS-сервер работает от root (EUID=0) — это нужно для сетевых операций. Но когда клиент запрашивает файл, сервер должен проверить права именно этого клиента, а не root. Если изменить EUID — сервер потеряет root-привилегии для всего остального. Решение: оставить EUID=0, но выставить fsUID=uid_клиента — тогда только файловые проверки пойдут от имени клиента. Изменяется через `setfsuid()`. В обычных процессах всегда равен EUID и менять его не нужно. |
...
**Шаг 2 — fork() и execve():**
Bash вызывает `fork()` — ядро создаёт точный клон процесса с теми же UID (1000/1000/1000). Это будущий процесс sudo. Клон (не исходный bash!) вызывает `execve("/usr/bin/sudo")` — системный вызов который **полностью заменяет текущий процесс новой программой**: тот же PID, но весь код, память и стек выбрасываются и заменяются бинарником sudo. Это не клонирование — клон уже существует после fork(), execve() просто превращает его в другую программу. Ядро при этом замечает SUID-бит (`-rwsr-xr-x`) и автоматически:
...
Теперь sudo имеет EUID=0: `open("/etc/sudoers")` — успешно (владелец root, права 440), вызов PAM, работа с `/etc/shadow`. Убедившись в разрешении, sudo вызывает `setuid(0)` (переводит и RUID в 0) → `fork()` → `execve("nginx")`. Дочерний процесс nginx-t получает все четыре UID = 0 и полноценно запущен от root.
...
```bash
# посмотреть все четыре UID процесса
...
> ⚠️ `usermod -G developers alice` **заменяет** все дополнительные группы пользователя на одну `developers`. Пользователь потеряет `sudo`, `docker` и всё остальное. Флаг `-a` (append) говорит «добавь к существующим группам» — без него `-G` перезаписывает весь список. Всегда используй `usermod -aG`.
...
Минимальное окружение — это не твоё окружение урезанное, а новое с нуля. sudo сам создаёт несколько переменных:
